...
class StochasticSubmodularPolicy(SubmodularPolicy):
    ...
    def update_history(self, vehicles, observed_value=None):
        ...
        1. if observed_value is None, then draw sample from each vehicle
        2. else, update observed distribution based on observed_values
        """
    ...
    def earth_mover(self):
        """ update constructed optimistic empirical distribution 
        ...
        while h_val < h_threshold:
            ...
            # improvement for Pr[A >= l]
                psi_val_updated = poisson_binomial_pmf([
                    ...
                    for v in recruited
                ], self.problem.l - 1) * self.problem.vehicles[v].get_arrival_probability(task_id) * np.mean(self.constructed[v]) + psi_val
                ...
                assert expected_h_val_updated != np.nan, (epsilon_p_j, epsilon_r_j, psi_val_updated)
