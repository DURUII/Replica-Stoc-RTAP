    ...
    def run(self, task_id: int, task: Task, remained: List[int]) -> None:
        ...
        for _ in range(max_inference_steps):
            ...
            if self.model is not None:
                ...
                full_action = self.model.select_action(obs, available_actions=available_full_actions, deterministic=False)
            ...
            if self.model:
                ...
                self.replay_buffer.add(prev_obs, int(action), float(reward), obs, bool(done))
                self._global_step += 1
